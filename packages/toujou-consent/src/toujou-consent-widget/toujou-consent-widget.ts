...
class ToujouConsentWidget extends LitElement {
  ...
  _handleConsentBoxSaveButtonClick(button) {
    if (button.hasAttribute('accept-all')) {
      this._prepareToSaveConsents(true);
    } else if (button.hasAttribute('deny-all')) {
      this._prepareToSaveConsents(false);
    } else {
      this._prepareToSaveConsents(null);
    }
    this._dismissConsentBox();
  }
